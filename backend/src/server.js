...
import {
  stripeConfigured, createCheckoutSession, constructWebhookEvent,
...
app.post("/webhooks/stripe", express.raw({ type: "application/json" }), (req, res) => {
  ...
  db.transaction(() => {
    ...
    if (event.type === "checkout.session.completed") {
      ...
      if (campaignId) {
        // Paiement encaissé → modération AUTOMATIQUE (diffusion immédiate sauf contenu interdit).
...
function validateBrandIcon(icon) {
  ...
  // 64 Ko de binaire ≈ 87 Ko de base64. On borne la chaîne pour être large mais sûr.
  if (icon.length > 90_000) return { ok: false };
...
app.post("/api/events", (req, res) => {
  ...
  const refusal = checkImpression(user.id);
...
app.get("/c/:campaignId", clickLimiter, (req, res) => {
  ...
  // Le clic vient du terminal du dev : on l'attribue au dernier dev ayant
  // affiché cette campagne depuis cette session si identifiable, sinon au
  // diffuseur courant. v1 simple : crédit au dev passé en ?u= si présent.
  const userId = req.query.u;
  if (userId && !killswitchOn()) {
    const user = db.prepare(`SELECT * FROM users WHERE id=? AND banned=0`).get(userId);
    if (user && !checkClick(user.id, campaign.id)) {
...
app.get("/auth/google/start", (req, res) => {
  ...
  // Destination web après login (ex. /me ou /annonceurs/espace). Chemin relatif
  // strict (commence par "/" sans "//") pour empêcher tout open-redirect.
  let next = typeof req.query.next === "string" ? req.query.next : "/me/";
  if (!/^\/[^/]/.test(next)) next = "/me/";
...
app.get("/auth/google/callback", async (req, res) => {
  ...
  try {
    ...
    if (!user) {
      user = { id: crypto.randomUUID(), email: profile.email, name: profile.name ?? "" };
      db.prepare(`INSERT INTO users (id,email,name,created_at) VALUES (?,?,?,?)`)
        .run(user.id, user.email, user.name, now());
    }
    const sessionToken = crypto.randomBytes(32).toString("hex");
    const expiresAt = now() + 90 * 24 * 60 * 60 * 1000; // 90 jours
...
app.post("/api/campaigns", async (req, res) => {
  ...
  if (stripeConfigured()) {
    db.prepare(
      `INSERT INTO campaigns (id,advertiser_email,advertiser_user_id,text,url,bid_cents,blocks,brand_name,brand_icon,show_on_leaderboard,status,created_at)
       VALUES (?,?,?,?,?,?,?,?,?,?, 'pending_payment', ?)`
    ).run(id, advertiserEmail, ownerId, text, url, bidCents, blocks, brandName, icon.value, showOnLeaderboard, now());
    try {
  ...
  db.prepare(
    `INSERT INTO campaigns (id,advertiser_email,advertiser_user_id,text,url,bid_cents,blocks,brand_name,brand_icon,show_on_leaderboard,status,created_at)
     VALUES (?,?,?,?,?,?,?,?,?,?,?,?)`
  ).run(id, advertiserEmail, ownerId, text, url, bidCents, blocks, brandName, icon.value, showOnLeaderboard, verdict, now());
  res.json({
